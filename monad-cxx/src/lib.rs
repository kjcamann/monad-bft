...
impl LogHandler {
    fn create(
        ...
        let mut c_handle: *mut ffi::c_void = std::ptr::null_mut();
...
static CXX_LOG_INIT: Once = Once::new();
static mut SINGLETON_LOG_HANDLER: Option<LogHandler> = None;

pub fn init_cxx_logging(log_level: Level) {
    CXX_LOG_INIT.call_once(|| {
        ...
        unsafe { SINGLETON_LOG_HANDLER.replace(handler) };
    });
}
