...
pub(crate) use self::bindings::{
    monad_evcap_event_iter, monad_evcap_event_section, monad_evcap_read_result, monad_evcap_reader,
    monad_evcap_section_desc, monad_evcap_section_type, MONAD_EVCAP_READ_END,
    MONAD_EVCAP_READ_NO_SEQNO, MONAD_EVCAP_READ_SUCCESS, MONAD_EVCAP_SECTION_EVENT_BUNDLE,
    MONAD_EVCAP_SECTION_LINK, MONAD_EVCAP_SECTION_NONE, MONAD_EVCAP_SECTION_PACK_INDEX,
    MONAD_EVCAP_SECTION_SCHEMA, MONAD_EVCAP_SECTION_SEQNO_INDEX,
};
...
#[inline]
fn get_last_evcap_library_error(r: libc::c_int) -> Result<(), String> {
    if r == 0 {
        return Ok(());
    }
...
pub(crate) fn monad_evcap_reader_create(
    ...
    get_last_evcap_library_error(r).map(|()| unsafe { &mut *c_evcap_reader })
}
...
pub(crate) fn monad_evcap_reader_refresh(
    ...
    get_last_evcap_library_error(r).map(|()| invalidated)
}

pub(crate) fn monad_evcap_reader_get_file_header<'r>(
    evcap_reader: &'r monad_evcap_reader,
) -> &'r monad_evcap_file_header {
    unsafe { &*self::bindings::monad_evcap_reader_get_file_header(evcap_reader) }
}

// pub(crate) fn monad_evcap_reader_get_mmap_base(evcap_reader: &mut monad_evcap_reader) -> &[u8] {
//     let r = unsafe {
//         self::bindings::monad_evcap_reader_get_mmap_base(evcap_reader)
//     }
// }

pub(crate) fn monad_evcap_reader_load_linked_section_desc<'r>(
    evcap_reader: &'r monad_evcap_reader,
    offset: u64,
) -> &'r monad_evcap_section_desc {
...
pub(crate) fn monad_evcap_reader_check_schema(
    ...
    get_last_evcap_library_error(r)
}
...
pub(crate) fn monad_evcap_event_section_open(
    ...
    get_last_evcap_library_error(r).map(|()| evcap_event_section)
}
...
// pub(crate) fn monad_evcap_event_section_at_offset(
//     evcap_event_section: &mut monad_evcap_event_section,
//     section_offset: u64,
// ) -> usize {
//     unsafe {
//         self::bindings::monad_evcap_event_section_at_offset(
//             evcap_event_section,
//             section_offset,
//             todo!(),
//             todo!(),
...
pub(crate) fn monad_evcap_event_section_copy_seqno<'s>(
    evcap_event_section: &'s monad_evcap_event_section,
    seqno: u64,
) -> (
    monad_evcap_read_result,
    Option<&'s monad_event_descriptor>,
    Option<&'s [u8]>,
) {
...
pub(crate) fn monad_evcap_event_iter_prev<'s>(
    evcap_event_iter: &'s mut monad_evcap_event_iter,
) -> (
    monad_evcap_read_result,
    Option<&'s monad_event_descriptor>,
    Option<&'s [u8]>,
...
pub(crate) fn monad_evcap_event_iter_copy<'s>(
    evcap_event_iter: &'s mut monad_evcap_event_iter,
) -> (
    monad_evcap_read_result,
    Option<&'s monad_event_descriptor>,
...
pub(crate) fn monad_evcap_event_iter_set_seqno<'s>(
    evcap_event_iter: &'s mut monad_evcap_event_iter,
    seqno: u64,
) -> monad_evcap_read_result {
