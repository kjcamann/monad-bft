...
/// An event capture reader created from an event capture file.
...
impl EventCaptureReader {
    ...
    /// Creates an [`EventCaptureReader`] from the ffi type.
    ///
    /// # Safety
    ///
    /// Callers of this method must guarantee that the `&'static mut monad_evcap_reader` lives on
    /// the heap and is never destroyed, since [`EventCaptureReader`] destroys the object in its
    /// drop method.
    ...
    /// Produces the next section descriptor in the event capture file, if any.
    pub fn next_section(
        &mut self,
    ...
    /// Produces the next event section descriptor in the event capture file, if any.
    pub fn next_event_section<D: EventDecoder>(
        &mut self,
    ) -> Option<EventCaptureEventSection<'_, D>> {
