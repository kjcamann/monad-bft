...
/// An iterator over the events in an event capture file event section.
    ...
    D: EventDecoder,
{
    ...
    /// Produces the next event descriptor in the event section.
...
/// A raw event capture payload
#[derive(Clone, Copy)]
pub struct EventCaptureRawPayload<'section> {
    /// The event section where the payload lives.
    pub event_section: &'section monad_evcap_event_section,
    /// A byte slice to the payload itself.
    pub payload: &'section [u8],
}

/// A marker type enabling [`EventDescriptorRead`] operations on event capture event descriptors.
