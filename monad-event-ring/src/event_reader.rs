...
use crate::{event_ring::*, event_ring_util::check_event_content_type};
...
impl<'ring> EventReader<'ring> {
    ...
    pub fn new(
        ...
        check_event_content_type(ring, content_type, schema_hash)?;
        ...
        Ok(EventReader {
            read_last_seqno,
            desc_capacity_mask: ring_contents.descriptor_capacity - 1,
    ...
    pub fn poll(&'_ mut self) -> PollResult {
        let ring_event: &monad_event_descriptor = &self.ring_contents.descriptors
            [(self.read_last_seqno as usize) & self.desc_capacity_mask];
    ...
    /// Read an event descriptor corresponding to a particular sequence
    /// number; returns None if the descriptor in the corresponding
    /// slot no longer has that sequence number
    #[inline]
    pub fn try_read_seqno(&'_ self, seqno: u64) -> Option<monad_event_descriptor> {
        if seqno == 0 {
            return None;
        }
        let index: usize = (seqno - 1) as usize & self.desc_capacity_mask;
        let ring_event: &monad_event_descriptor = &self.ring_contents.descriptors[index];
        let ring_seqno_ptr: *const u64 = &ring_event.seqno as *const u64;
        let ring_seqno: &AtomicU64 = unsafe { AtomicU64::from_ptr(ring_seqno_ptr as *mut u64) };
        let event = *ring_event;
        if ring_seqno.load(Ordering::Acquire) != seqno {
            None
        } else {
            Some(event)
        }
    ...
    fn sync_wait(&'_ mut self) -> u64 {
        const MAX_SYNC_SPIN: u64 = 100;
        let mut write_last_seqno = self.ring_contents.write_last_seqno.load(Ordering::Acquire);
        ...
        while write_last_seqno > 0 {
            let index: usize = (write_last_seqno - 1) as usize & self.desc_capacity_mask;
            let ring_event: &monad_event_descriptor = &self.ring_contents.descriptors[index];
            let ring_seqno_ptr: *const u64 = &ring_event.seqno as *const u64;
            let ring_seqno: &AtomicU64 = unsafe { AtomicU64::from_ptr(ring_seqno_ptr as *mut u64) };
            let mut spin_counter: u64 = 0;
            while spin_counter < MAX_SYNC_SPIN {
                if ring_seqno.load(Ordering::Acquire) == write_last_seqno {
