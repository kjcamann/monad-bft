...
/// constant `enum monad_event_content_type` in event_ring.h
...
/// Describes an error encountered during event recording
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
#[repr(u16)]
pub enum RecordError {
    NoError,         // Invalid value
    Overflow4GB,     // Payload exceeded UINT32_MAX
    OverflowExpire,  // Payload expired immediately at creation
}
...
impl EventRing {
    ...
    /// Returns what kind of content this event ring contains, along with the
    /// SHA-256 hash of the content schema, indicating the binary version of
    /// the content protocol
    pub fn get_content_type(&self) -> (EventContentType, &[u8; 32]) {
        let rt = unsafe { &(*self.c_event_ring.header) }.event_content_type;
        if rt >= EventContentType::Count as u16 {
            return (EventContentType::Invalid, &Self::ZERO_HASH);
        };
        let header: &monad_event_ring_header = unsafe { &(*self.c_event_ring.header) };
        (
            unsafe { std::mem::transmute::<u16, EventContentType>(rt) },
            &header.schema_hash,
        )
    }
...
#[repr(C, align(16))]
pub struct monad_event_record_error {
    pub error_type: u16,
...
pub(crate) struct monad_event_ring {
    ...
    pub(crate) desc_capacity_mask: u64,
    pub(crate) payload_buf_mask: u64,
}
...
extern "C" {
    ...
    static g_monad_event_content_type_names: *const *const libc::c_char;
}
