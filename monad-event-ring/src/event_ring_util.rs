...
/// Validate that an event ring has the expected content type and schema hash
pub fn check_event_content_type(
    ring: &EventRing,
    content_type: EventContentType,
    schema_hash: &'_ [u8; 32],
) -> Result<(), String> {
    let r = unsafe {
        monad_event_ring_check_content_type(&ring.c_event_ring, content_type as u16, schema_hash.as_ptr())
    };
...
extern "C" {
    fn monad_event_ring_check_content_type(
    ...
    fn monad_event_open_ring_dir_fd(
        dirfd: *mut libc::c_int,
        namebuf: *mut libc::c_char,
        namebuf_size: usize) -> libc::c_int;
}
...
impl EventRingSnapshot {
    pub fn load_from_bytes(bytes: &[u8], error_name: &str) -> EventRingSnapshot {
        ...
        if u32::from_le_bytes(magic_bytes) == zstd::zstd_safe::MAGICNUMBER {
            ...
            let mut copy_out = bytes;
            while !copy_out.is_empty() {
                let n_write = unsafe {
                    libc::write(
                        snapshot_fd,
                        copy_out.as_ptr() as *const libc::c_void,
                        copy_out.len() as libc::size_t,
                    )
                };
                assert_ne!(n_write, -1);
                copy_out = &copy_out[n_write as usize..];
            }
        };
...
/// ring.
pub fn open_event_ring_file(input: impl AsRef<Path>) -> Result<OpenedEventRing, String> {
    let mmap_prot = libc::PROT_READ;
    let mut event_ring_path: PathBuf;

    if input.as_ref().components().count() == 1 {
        ...
        let rc = unsafe { monad_event_open_ring_dir_fd(
            std::ptr::null_mut(),
            ring_dir_buf.as_mut_ptr() as *mut libc::c_char,
            ring_dir_buf.len())
        };
        check_event_ring_library_error(rc)?;
        let ring_dir_cstr = match CStr::from_bytes_until_nul(ring_dir_buf.as_slice()) {
            Ok(cstr) => cstr,
            Err(err) => return Err(err.to_string()),
        };
        event_ring_path = PathBuf::from(ring_dir_cstr.to_str().unwrap());
        event_ring_path = event_ring_path.join(input);
    }
    else {
        event_ring_path = PathBuf::from(input.as_ref());
    }
