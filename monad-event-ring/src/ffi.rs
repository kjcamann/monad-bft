...
pub(crate) fn monad_event_set_ring_dir_override(
    ...
    let r = if let Some(path_ref) = opt_path_ref {
        let path_str = match path_ref.as_ref().to_str() {
            Some(s) => s,
            None => return Err(String::from("path contained invalid UTF-8 characters")),
        };
        let path_cstring = CString::new(path_str).map_err(|nul_err| nul_err.to_string())?;
        unsafe { self::bindings::monad_event_set_ring_dir_override(path_cstring.as_ptr()) }
    } else {
        unsafe { self::bindings::monad_event_set_ring_dir_override(std::ptr::null()) }
    };
    get_last_ring_library_error(r)?;
    Ok(())
}
