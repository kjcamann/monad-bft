...
/// A marker type enabling [`EventDescriptorRead`] operations on event ring event descriptors.
pub struct EventRingPayload<'ring> {
    _phantom: PhantomData<&'ring RawEventRing>,
}

impl<'buf> EventDescriptorRead for EventRingPayload<'buf> {
