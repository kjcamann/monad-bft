    ...
    D: EventDecoder,
{
    ...
    /// be discarded, which is expressed through the event source's "expired" variant. As such, there
    /// are some event sources that *could* have this requirement relaxed but this would require
    /// unsafe or a more complex API.
    ///
...
where
    Self: Sized,
{
    /// The associated raw event descriptor type.
    type Raw;

    /// The result type associated with this event source.
    type Result<T>;
    ...
    /// Creates an "ok" variant of [`Self::Result`](EventDescriptorRead::Result).
    fn result_ok<T>(value: T) -> Self::Result<T>;

    /// Provides `map` functionality for [`Self::Result`](EventDescriptorRead::Result).
    fn result_map<T, U>(result: Self::Result<T>, f: impl FnOnce(T) -> U) -> Self::Result<U>;

    /// Provides `and_then` functionality for [`Self::Result`](EventDescriptorRead::Result).
...
where
    D: EventDecoder,
{
    /// Creates a new [`EventDescriptorInfo`].
