...
use monad_exec_events::{
    ...
    exec_event_ctypes::{EXEC_EVENT_DEFAULT_FILE_NAME, EXEC_EVENT_DOMAIN_METADATA},
    exec_event_stream::*,
    exec_events::{BlockTag, ConsensusState, ExecEvent},
};
...
fn print_failed_block_info(fail_info: Box<FailedBlockInfo>, display: DisplayFormat) {
    match display {
        DisplayFormat::Summary => {
            ...
            let block_id = fail_info.block_tag.id.0;
...
fn print_executed_block_info(
    ...
    match display {
        DisplayFormat::Summary => {
            ...
            let block_id = exec_info.block_tag.id.0;
...
fn print_referendum_update(
    ...
    match display {
        ...
        DisplayFormat::Debug => println!(
            ...
            ExecEvent::Referendum { block_tag, outcome }
        ),
        DisplayFormat::Json => println!(
            "{}",
            serde_json::to_string_pretty(&ExecEvent::Referendum { block_tag, outcome }).unwrap()
        ),
    }
}
...
fn print_abandoned_proposals(
    ...
    for abandoned_block in abandoned_proposals {
        ...
        let number = abandoned_block.block_tag.block_number;
        let block_id = abandoned_block.block_tag.id.0;
            ...
            Some(BlockUpdate::NonBlockEvent(ExecEvent::Referendum { block_tag, outcome })) => {
                ...
                if let Some(pending_block) = consensus_tracker.get(block_number, block_id) {
                    ...
                    match outcome.cmp(&earliest_consensus_state) {
                        ...
                        Ordering::Greater => {
                            print_referendum_update(block_tag, outcome, display, false);
                        }
                    }
                    ...
                    print_referendum_update(block_tag, outcome, display, true);
                }
                if let ConsensusStateResult::Finalization {
                    finalized_proposal: _,
                    abandoned_proposals,
