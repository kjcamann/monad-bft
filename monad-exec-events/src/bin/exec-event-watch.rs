...
use monad_exec_events::exec_event_ctypes::{
    self, exec_event_type, flow_type, EXEC_EVENT_DEFAULT_FILE_NAME, EXEC_EVENT_DOMAIN_METADATA,
};
...
struct Cli {
    ...
    #[arg(default_value = EXEC_EVENT_DEFAULT_FILE_NAME)]
...
fn load_block_number(reader: &EventReader, block_start_seqno: u64) -> Option<u64> {
    if let Some(start_desc) = reader.try_read_seqno(block_start_seqno) {
        use exec_event_ctypes::block_start;
        let block_start: *const block_start =
            reader.payload_peek(&start_desc).as_ptr() as *const block_start;
        let block_number = unsafe { (*block_start).block_tag.block_number };
        if reader.payload_check(&start_desc) {
            Some(block_number)
        } else {
            None
        }
...
{event_code:#x}] SEQ: {event_seqno} LEN: {event_size} BUF_OFF: {event_buf_offset}"
    ...
    let block_start_seqno = event.content_ext[flow_type::BLOCK_SEQNO as usize];
    ...
    match (block_start_seqno, txn_id) {
        ...
        (block_start_seqno, 0) => {
            let opt_block_number: Option<u64> = load_block_number(reader, block_start_seqno);
            if let Some(block_number) = opt_block_number {
                event_line.push_str(format!(" BLK: {block_number}\n").as_str())
            } else {
                event_line.push_str(format!(" BLK: <LOST>\n").as_str())
            }
        }

        // Event is associated with a transaction in a block; print both
        // summary block info and the transaction number
        (block_start_seqno, txn_id) => {
            let opt_block_number: Option<u64> = load_block_number(reader, block_start_seqno);
            let txn_num = txn_id - 1;
            if let Some(block_number) = opt_block_number {
                event_line.push_str(format!(" BLK: {block_number} TXN: {txn_num}\n").as_str())
            } else {
                event_line.push_str(format!(" BLK: <LOST> TXN: {txn_num}\n").as_str())
            }
        }
    }
    ...
    if decode_payload && event.payload_size > 0 {
