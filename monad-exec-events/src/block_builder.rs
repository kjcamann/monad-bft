...
use crate::exec_events::{
    AccountAccess, AccountAccessContext, BlockTag, CallFrame, ConsensusState, EthBlockInput,
...
impl BlockBuilder {
    ...
    pub fn try_append(&mut self, event: ExecEvent) -> Option<BlockUpdate> {
        match event {
            ExecEvent::BlockStart {
                ...
                let txn_count = eth_block_input.transaction_count as usize;
                self.current_block.replace(Box::new(ExecutedBlockInfo {
                    consensus_state,
                    block_tag,
                    chain_id,
                    eth_header: create_alloy_block_header(eth_block_input),
            ...
            txn_event @ ExecEvent::TransactionAccessListEntry { .. }
            | txn_event @ ExecEvent::TransactionAuthorizationListEntry { .. }
            | txn_event @ ExecEvent::TransactionHeaderEnd { .. }
            | txn_event @ ExecEvent::TransactionEvmOutput { .. }
            ...
            ExecEvent::TransactionHeaderStart {
                ...
                authorization_list_entry_count,
            } => {
    ...
    fn append_txn_event(&mut self, event: ExecEvent) -> Option<BlockUpdate> {
        ...
        let txn_index = *match &event {
            ExecEvent::TransactionHeaderStart { txn_index, .. } => txn_index,
            ExecEvent::TransactionAccessListEntry { txn_index, .. } => txn_index,
            ExecEvent::TransactionAuthorizationListEntry { txn_index, .. } => txn_index,
            ExecEvent::TransactionHeaderEnd { txn_index, .. } => txn_index,
        ...
        match event {
            ExecEvent::TransactionAccessListEntry {
                ...
                match txn_info.txn_envelope {
                    ...
                    alloy_consensus::TxEnvelope::Eip4844(ref mut txn_eip_4844_variant) => {
                        if let alloy_consensus::TxEip4844Variant::TxEip4844(txn_eip_4844) =
                            txn_eip_4844_variant.tx_mut()
                        {
                            txn_eip_4844.access_list.0.push(entry);
                        }
                    }
                    alloy_consensus::TxEnvelope::Eip7702(ref mut txn_eip_7702) => {
                        txn_eip_7702.tx_mut().access_list.0.push(entry)
                    }
                    _ => (),
                };
            }
            ...
            ExecEvent::TransactionAuthorizationListEntry {
                txn_index: _,
                authorization_list_index: _,
                entry,
                authority: _,
            } => {
                if let alloy_consensus::TxEnvelope::Eip7702(ref mut txn_eip_7702) =
                    txn_info.txn_envelope
                {
                    txn_eip_7702.tx_mut().authorization_list.push(entry)
                };
            }

            ExecEvent::TransactionHeaderEnd { .. } => {
                // TODO(ken): ignore this for now, could use it to check things
            }
...
fn create_alloy_block_header(exec_input: EthBlockInput) -> alloy_consensus::Header {
