...
use crate::exec_events::{BlockTag, ConsensusState, MonadBlockId};
...
impl<T> ConsensusStateTracker<T> {
    ...
    pub fn add_proposal(
        ...
        match self.proposal_state_map.entry(block_tag.block_number) {
            hash_map::Entry::Occupied(mut proposal_tokens) => {
                ...
                if let Some(dup) = proposal_tokens
                    .get()
                    .iter()
                    .find(|ps| ps.block_tag.id == block_tag.id)
                {
                    ...
                    return ConsensusStateResult::Outstanding {
                        ...
                        other_proposals: proposal_tokens
                            .get()
                            .iter()
                            .filter(|ps| ps.block_tag.id != block_tag.id)
                            .cloned()
                            .collect(),
                    };
                }
                ...
                match initial_state {
                    ...
                    ConsensusState::Finalized => {
                        ...
                        self.handle_finalized(&block_tag.id, proposals)
                    }
                ...
                match new_state {
                    ...
                    ConsensusState::QC => {
                        ...
                        let (updated, remaining): (Vec<_>, Vec<_>) = proposal_tokens
                            .iter()
                            .partition(|pt| pt.block_tag.id == *block_id);
    ...
    pub fn get(&self, block_number: u64, block_id: &MonadBlockId) -> Option<Arc<ProposalState<T>>> {
        if let Some(pending_block_tokens) = self.proposal_state_map.get(&block_number) {
            pending_block_tokens
                .iter()
                .find(|pbt| pbt.block_tag.id == *block_id)
                .cloned()
        } else {
            None
        }
    ...
    fn handle_finalized(
        ...
        let (finalized_proposals, abandoned_proposals): (Vec<_>, Vec<_>) = proposal_tokens
            .into_iter()
            .partition(|pt| pt.block_tag.id == *finalized_block_id);
