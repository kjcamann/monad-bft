...
impl<'ring> ExecEventStream<'ring> {
    ...
    fn act_on_block_start(&'_ mut self, event: monad_event_descriptor) -> PollResult {
        ...
        let extra_data = &block_start.exec_input.extra_data.as_slice()
            [..block_start.exec_input.extra_data_length as usize];
        self.try_create_event(
            ...
            ExecEvent::BlockStart {
                ...
                exec_input: EthBlockExecInput {
                    ...
                    ommers_hash: block_start.exec_input.ommers_hash,
                    beneficiary: block_start.exec_input.beneficiary,
                    transactions_root: block_start.exec_input.transactions_root,
                    difficulty: block_start.exec_input.difficulty,
                    number: block_start.exec_input.number,
                    gas_limit: block_start.exec_input.gas_limit,
                    timestamp: block_start.exec_input.timestamp,
                    extra_data: Bytes::copy_from_slice(extra_data),
                    prev_randao: block_start.exec_input.prev_randao,
                    nonce: B64::new(block_start.exec_input.nonce),
                    base_fee_per_gas: match block_start.exec_input.base_fee_per_gas {
                        U256::ZERO => None,
                        f => Some(f),
                    },
                    withdrawals_root: match block_start.exec_input.withdrawals_root {
                        B256::ZERO => None,
                        r => Some(r),
                    },
                    transaction_count: block_start.exec_input.txn_count,
                },
            },
        )
    }
