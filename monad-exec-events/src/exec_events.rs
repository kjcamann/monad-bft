...
pub enum ExecEvent {
    /// Event that indicates an error occurred during the event recording
    /// TODO(ken): error_type and dropped_event_type are unstructured
    ///   because of serialization
    RecordError {
        error_type: u16,
        dropped_event_type: u16,
        requested_payload_size: usize,
    ...
    /// Performance marker event emitted to mark the beginning of the EVM
    /// transaction processing portion of block execution; this indicates that
    /// sender recovery and basic block validation has completed
    BlockPerfEvmEnter,

    /// Performance marker event emitted to mark the end of the EVM transaction
    /// processing portion of block execution; the final BlockEnd event will
    /// occur later (unless EvmError occurs first); this event has no payload
    /// and is only used to compute the db commit time
    BlockPerfEvmExit,
    ...
    Referendum {
        /// Tag for block proposal this referendum relates to
        block_tag: BlockTag,
        /// The new consensus state of the block proposal, as set by the referendum
        outcome: ConsensusState,
    },

    /// Event that marks the start of the transaction header (the variable
    /// length items come as separate events)
    TransactionHeaderStart {
        ...
        /// access list and EIP-7702 authorization list because they are
        /// announced in subsequent events
        ...
        /// Number of TransactionAuthorizationEntry events that will
        /// immediately follow the TransactionAccessListEntry events
        authorization_list_entry_count: u32,
    },

    /// Event emitted for a single entry in an EIP-2930 list; these are emitted
    /// in index order immediately following the TransactionHeaderStart event
    ...
    /// Event emitted for a single entry in an EIP-7702 list; these are emitted
    /// in index order immediately following the TransactionHeaderStart event
    TransactionAuthorizationListEntry {
        txn_index: u64,
        authorization_list_index: u32,
        entry: eip7702::SignedAuthorization,
        authority: Option<Address>,
    },

    /// Marks the end of the transaction header
    TransactionHeaderEnd { txn_index: u64 },
    ...
    /// Performance marker event emitted when a transaction begins execution
    /// in the EVM (after sender recovery has completed)
    TransactionPerfEvmEnter {
        /// Index of transaction which has begun execution
        txn_index: u64,
    },

    /// Performance marker event emitted when a transaction completes execution
    /// in the EVM
    TransactionPerfEvmExit {
        /// Index of transaction which has begun execution
        txn_index: u64,
    },
    ...
    /// etc.) are reported after this one. This event serves as a "header"
    ...
    /// Event emitted to mark the end of all events related to a particular
    /// transaction; not emitted if TransactionReject or EvmError occur
    TransactionEnd { txn_index: u64 },
